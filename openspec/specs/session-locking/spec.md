# session-locking Specification

## Purpose

The session-locking contract: every host runs a hyprlock session
locker with PAM auth, hypridle locks after 10 minutes of idle and
locks-before-suspend so resume lands on the lock screen, and
SUPER+l triggers a manual lock from any active Hyprland session.
The lock UI is a single declarative config file
(`modules/hyprland/hyprlock-themed.conf`) wired via
`xdg.configFile."hypr/hyprlock.conf".source`; the display manager
(ly) is never involved in session locking.
